...
##
## Case Name: check-keyword-detection-in-D0i3
## Preconditions:
##    N/A
## Description:
##    Run arecord on kwd pipeline first, wait DSP enters to D0i3, then play a
##    a special sine wav(997 Hz sine wav which generated by wav tool) to trigger
##    kwd and then use wavtool to do the analysis.
## Case step:
##    1. use wavtool to generate the test wav file.
##    2. run kwd first and wait DSP enters to D0i3.
##    3. check the DSP status.
##    4. play the test wav through the USB card to trigger kwd.
##    5. after the test, use wavetool to analyze the recorded wav.
## Expect result:
...
do
    ...
    do
        ...
        # run arecord on keyword detection pipeline first
        dlogc "arecord -D$dev -M -N -r $rate -c $channel -f $fmt --buffer-size=$buffer_size -d $duration $recorded_file"
        arecord -D"$dev" -M -N -r "$rate" -c "$channel" -f "$fmt" --buffer-size="$buffer_size" -d "$duration" \
		"$recorded_file" -vvv 2>/dev/null & WOVPID=$! 
        
	sleep 6 # wait for DSP enter to D0i3
	# check D0i3 status
	sudo hexdump /sys/kernel/debug/sof/hda |grep "0001040" |awk -F ' ' '{print $7}' |grep "04" || die "DSP failed to enter D0i3"

        # play the test wav to trigger kwd
        dlogc "aplay -Dplughw:1,0 $test_file"
        aplay -Dplughw:1,0 "$test_file"  || die "Failed to play the test wav on USB sound card"
        dlogi "Testing: iteration $i with $fmt format"

        timeout 5 tail --pid="$WOVPID" -f /dev/null || die "Failed to stop arecord on kwd pipeline"
