...
##
## Case Name: check-pause-resume-simultaneous-multiplayback-hdmi-analog
## Preconditions:
##    N/A
## Description:
##    test playback with pause/resume on 2 separate simultaneous HDMI & Analog
##      pipelines
...
OPT_OPT_lst['H']='pcm_HDMI'     OPT_DESC_lst['H']='HDMI pcm for playback. Example: hw:0,0'
OPT_PARM_lst['H']=1             OPT_VALUE_lst['H']=''
...
# HDMI playback pcm settings
pcm_HDMI=${OPT_VALUE_lst['H']}

# ANALOG playback pcm settings
pcm_ANALOG=${OPT_VALUE_lst['A']}

if [ "$pcm_HDMI" = "" ]||[ "$pcm_ANALOG" = "" ];
then
...
dlogi "Entering HDMI playback expect script with: $cmd -D $pcm_HDMI -r $rate -c $channel -f $fmt -vv -i $dummy_file -q"
...
expect <<HDMI &
spawn $cmd -D $pcm_HDMI -r $rate -c $channel -f $fmt -vv -i $dummy_file -q
set i 1
expect {
    "*#*+*\%" {
        set sleep_t [expr int([expr rand() * $rnd_range]) + $rnd_min ]
        puts "\r(\$i/$repeat_count) HDMI: Wait for \$sleep_t ms before pause"
        send " "
        after \$sleep_t
        exp_continue
    }
    "*PAUSE*" {
        set sleep_t [expr int([expr rand() * $rnd_range]) + $rnd_min ]
        puts "\r(\$i/$repeat_count) HDMI: Wait for \$sleep_t ms before resume"
        send " "
        after \$sleep_t
        incr i
