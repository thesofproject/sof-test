...
## Test Description
* Verification of audio privacy (hard mute) functionality during DSP D3 state in linux system.
...
2. Run command in second terminal to monitor changes of DSP state via command:
    ```bash
    dmesg --follow | grep DSP
    ```
    By default DSP is in D3 state and after interrupt (play or record new wav file, DSP wakes up and is in active state D0I0)
3. Make sure that DSP is in D3 state, then turn ON the audio privacy switch. Led indicator must be turned ON.
4. Capture audio via 3.5mm jack ports:
    ```bash
    arecord -Ddefault -c 2 -r 48000 -f S16_LE /tmp/test_sample_mute_during_d3.wav -vvv
    ```
5. Check if DSP was in actvie state "D0I0" during recording, then check on the file if audio was muted in any application to verify sound (for example Audacity).
6. Make sure that DSP is in D3 state, then turn OFF the audio privacy switch. Led indicator must be turned OFF.
7. Capture audio via 3.5mm jack ports:
    ```bash
    arecord -Ddefault -c 2 -r 48000 -f S16_LE /tmp/test_sample_unmute_during_d3.wav -vvv
    ```
8. Check if DSP was in actvie state "D0I0" during recording, then check on the file if audio was not muted in any application to verify sound (for example Audacity).
9. Check for errors: 
    ```bash
    journalctl -b -p 4.
    ```
...
2. DSP must be in D3 state.
3. Recording must start without problems.
4. The MIC mute LED must glow.
5. During recording DSP must be in active state(D0I0), after recording, audio must be muted.
6. DSP must be in D3 state.
7. The MIC mute LED must turn OFF.
8. Recording must start without problems.
9. During recording DSP must be in active state(D0I0), after recording, audio must be unmuted
