...
check_tplg()
{
    ...
    hda_tplg="sof-hda-benchmark-micsel_multich32.tplg"
    nocodec_tplg="sof-ptl-nocodec.tplg"
    tplg_dir="/lib/firmware/intel/development"

    if [[ "$tplg" == *"hda"* && "$tplg" != "$tplg_dir/$hda_tplg" ]]; then
        dlogi "HDA topology change required"
        new_tplg_filename=$hda_tplg
        update_topology_filename
    elif [[ "$tplg" == *"nocodec"* && "$tplg" != "$tplg_dir/$nocodec_tplg" ]]; then
        dlogi "NO-CODEC topology change required"
        new_tplg_filename=$nocodec_tplg
        update_topology_filename
    elif [[ "$tplg" != *"nocodec"* && "$tplg" != *"hda"* ]]; then
        skip_test "This test is for HDA or NO-CODEC platforms only"
    fi
    
    dlogi "Topology correct for the test"
}
...
{
    ch_nr="$1"
    if python3 ~/sof-test/tools/test-sound-generator.py "$1"; then
