...
def state_str(state):
    ...
    pipeline creation from the other pause state changes that actually
    do something.  '''
    if state == 10:
        return "1.PAUSED"
    return f'<bad atate {state}>'
...
class IpcMsgParser(LogLineParser):
    ...
    def parse_multi_pipe_trigger_params_line(self, line):
        if line.find(":sof_ipc4_set_multi_pipeline_state:") >= 0:
            ...
            end_pos = line.find(" state switching to ")
    ...
    def mod_msg_parse_1st(self, usecs, primary):
    ...
    def mod_init_msg_parse(self, msg_type, msg_str, usecs, primary):
        if msg_type == "     ":
            self.mod_msg_parse_1st(usecs, primary)
